...
Use Bobbin's embedded core only; Core alone owns final validation and writes. Apart from the shared recording policy, do not read `references/`, plugin manifests, or `context/*.index.md`: `search` reads metadata; `compare` reads selected comparison bodies. Never run `--help` and never read or grep plugin scripts; use the complete commands below, apply an actionable error's correction before retrying, and inspect script source only after an unexplained interface failure. Never add an attestation unless the required actual comparison supports it. Use the active language for user text and English for machine fields; do not translate artifact prose.

## Recall and decide

1. Run only on core's choice signal: the user's own stated or changing choice. A compatible task needs no follow-up comparison. Reuse a valid result while the same scope, anchor and complete actual bodies remain in context and no relevant mutation is known; never add a call to prove reuse. If scope/key are known, run one `decision_cli.mjs compare`. Otherwise use one metadata-only `search --query '<distinctive term>'`, adding `--scope '<scope>'` when known. Select coordinates from metadata, then compare actual bodies before judging; do not invent an alias for an existing choice. Empty search results do not prove absence and do not trigger automatic global searches. Missing scope requires clarification; a genuinely new choice may use a caller-provided new key and its exact comparison.
2. Reuse sections returned by `compare` in the same turn under those conditions; do not call `read`, `spec-view`, or another context read unless required actual content is absent or a relevant mutation is known. Compare actual `Decision`, `Rationale`, `Rejected alternatives`, and non-empty `Revisit conditions`. Classify `new|same|supporting|rationale_changed|conflict`; similarity, hashes, IDs, and metadata are not evidence. For the same choice and governing scope, reuse the returned `scope` and `decision_key`; do not invent an alias.
...
The stable `context-decision-compare/v1` envelope references `context-decision-assessment/v1`, defined by the rules above and `decision_cli.mjs schema --json` for clients without this skill. Do not reload that contract for each call. Inspect `retrieval` and `warnings`: the comparison covers only the selected scoped set. If relevant evidence is missing, use metadata or an explicit ID read, not an automatic broad rerun.

On a necessary later compare, pass `--known-current '<id>:<returned-sha256>'` for complete actual comparison sections still held in this scope/anchor context (at most 12 unique IDs). Copy the returned `sha256:` prefix. Resolve `comparison.current[].sections_ref` from those actual sections before comparing or quoting. Partial reads, context loss or handoff require omitting the flag. New or changed files return sections. `hydrated_input_digest` binds the full comparison input and `transport_digest` binds `comparison`; neither proves meaning or approval. Full semantic limits still apply. Legacy `check` and its full/delta envelopes remain compatibility surfaces.

```bash
node /loaded/bobbin/skills/decision/scripts/decision_cli.mjs search \
  --query '<distinctive term>' --scope '<scope>' --json
...
Use the shared recording policy. Do not pre-run host inventory or core doctor. Run one `record --approved` for user approval, or `record --approval-source policy` for configured automation; adaptive also requires a record/ask assessment and reason. Internal preview and unchanged apply bind `approval_digest` privately; never expose or request transport details. The result confirms the write: do not compare again afterwards.
