...
Replace with `record --supersede <current-id>`, retire with `record --withdraw <current-id> --reason <text>`, and discard a pending low-level receipt with `reject --receipt-file <retained-path>`. History is `do_not_follow`. Low-level orchestration: `decision_workflow.mjs preview` (frozen receipt; `preview stdout` carries `approval_digest`) then `apply --approved-digest`.
