...
- Common primary-claim protocol ceiling: 2,000 codepoints; built-in OBS `observation`: 1,200 codepoints; canonical owner input: 8 KiB. SNAP uses the separate storage bounds below.
...
- Read budgets remain separate from explicit storage. ARCHIVE capture allows immutable `Content` of at most 65,000 codepoints and a 512 KiB candidate, owner-input and approval-preview envelope ceiling.
- SNAP content has no separate paragraph, character, list-count or item-length ceiling. Its complete logical payload is at most 262,144 UTF-8 bytes of compact `JSON.stringify({title,summary,captured_from,source_refs,tags,search_terms,anchors,sections})`; absent lists are `[]`, and `sections` maps canonical ordered section names to raw text. Metadata bounds and required structure remain enforced. Create and update, including the merged result of a partial update, use the same check. `snapshot_input_too_large` reports `actual_bytes`, `max_bytes` and `measurement: snapshot_payload_utf8`, with no partial record/index write or automatic shortening/splitting.
- SNAP candidate JSON is at most 540,672 bytes; a capture batch or approval preview containing SNAP may use 1 MiB, while its non-SNAP subset retains the existing 16 KiB batch / 128 KiB preview budget. The JSON/receipt transport ceiling remains 2 MiB. These storage exceptions do not change search/recall budgets or other record kinds' limits; explicit SNAP load/read returns full content unless a read limit is supplied.
- SNAP preserves Markdown content, Unicode, indentation and trailing spaces, normalizing CRLF to LF. When legacy H2 sections cannot round-trip losslessly, collision-free HTML section markers and `section_delimiter` frame the body; an already framed SNAP keeps framing on update. Existing SNAPs remain readable without automatic migration. Older runtimes may not read newly framed or larger SNAPs reliably.
