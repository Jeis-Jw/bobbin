...
Bobbin 2.1.1의 설치·설정은 단일 `$bobbin:init`을 사용한다. 아래 사용자 승인 설명은 explicit 모드 기준이며, auto/adaptive는 [공통 기록 정책](recording-policy.md)에 따라 같은 검증 경로를 사용한다. 사용자 의사와 의미 검증은 승인 모드와 무관하게 유지한다. 기존 owner·schema 식별자는 내부 호환 계약이며 별도 설치 단위가 아니다.
...
- runtime 경계는 healthy Stage 1의 index-only read와 artifact open/list/stat 0, recovery body open 최대 20개, Stage 1 4 KiB·body pack 8 KiB, common primary-claim protocol 2,000 codepoint, built-in OBS `observation` 1,200 codepoint, 최대 8개이면서 schema·audit_count·candidates 전체 canonical UTF-8 envelope가 16 KiB 이하인 `context-capture-batch/v1`, owner input 8 KiB, 일반 approval preview 32 KiB 상한으로 집행한다. 읽기 예산과 명시적 저장 상한은 구분한다. ARCHIVE capture는 불변 `Content` 65,000 codepoint와 candidate·owner input·approval preview 512 KiB envelope을 허용하며, SNAP에는 아래 저장 예외를 적용한다. Dict batch는 세 key만 허용하고 audit_count는 bool이 아닌 integer `1`이어야 한다. legacy bare list는 같은 synthetic v1 envelope byte budget으로 유지한다. zero-match의 unindexed 확인은 directory entry 수에 따라 증가할 수 있지만 indexed artifact body는 열지 않는다. `context-decision`은 별도로 decision 1,200 codepoint, brief 8 KiB·comparison input 24 KiB·result 32 KiB 상한을 둔다.
- SNAP 내용의 문단·글자 수·목록 개수·항목 길이 제한은 별도로 두지 않는다. 전체 논리적 payload는 compact `JSON.stringify({title,summary,captured_from,source_refs,tags,search_terms,anchors,sections})`의 UTF-8 크기로 262,144바이트 이하이며, 없는 목록은 `[]`, `sections`는 정해진 순서의 canonical 섹션 이름과 원문 텍스트 map이다. 메타데이터 제한과 필수 구조는 유지한다. 생성·갱신과 부분 갱신을 합친 최종 내용에 같은 검증을 적용한다. 초과 시 `snapshot_input_too_large`와 `actual_bytes`, `max_bytes`, `measurement: snapshot_payload_utf8`를 반환하고, 기록·색인의 부분 저장이나 자동 축약·분할은 하지 않는다.
- SNAP candidate JSON은 540,672바이트 이하이며, SNAP을 포함한 capture batch·approval preview는 1 MiB를 허용한다. 이때 non-SNAP 하위 묶음은 기존 batch 16 KiB·preview 128 KiB 예산을 유지한다. JSON/receipt transport 상한 2 MiB는 유지한다. 이 저장 예외는 search/recall 예산이나 다른 기록 유형의 제한을 바꾸지 않으며, 명시적 SNAP load/read는 별도 조회 제한이 없으면 전체 내용을 반환한다.
- SNAP은 Markdown·Unicode·들여쓰기·줄 끝 공백을 보존하고 CRLF를 LF로 바꾼다. 기존 H2 섹션 형식으로 손실 없이 왕복할 수 없으면 `section_delimiter`와 충돌 없는 HTML 섹션 마커로 본문을 구분하며, 이미 이 형식이면 갱신에서도 유지한다. 기존 SNAP은 자동 변환 없이 읽을 수 있다. 구버전 runtime은 새 마커 형식이나 커진 SNAP을 제대로 읽지 못할 수 있다.
