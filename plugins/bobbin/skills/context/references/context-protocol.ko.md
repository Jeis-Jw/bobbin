...
Bobbin 2.1.0의 설치·설정은 단일 `$bobbin:init`을 사용한다. 아래 사용자 승인 설명은 explicit 모드 기준이며, auto/adaptive는 [공통 기록 정책](recording-policy.md)에 따라 같은 검증 경로를 사용한다. 사용자 의사와 의미 검증은 승인 모드와 무관하게 유지한다. 기존 owner·schema 식별자는 내부 호환 계약이며 별도 설치 단위가 아니다.
