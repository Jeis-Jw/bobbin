...
활성 host에 맞춰 `context_cli.mjs init --host codex --json` 또는 `context_cli.mjs init --host claude-code --json`을 한 번 호출한다. absent repository에는 canonical root/SNAP/OBS seed와 `codex → AGENTS.md`, `claude-code → CLAUDE.md` 관리형 policy를 설치한다. ready 재호출은 noop이다.
