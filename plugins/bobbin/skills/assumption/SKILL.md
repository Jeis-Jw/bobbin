...
For capture, use sibling `scripts/assumption_workflow.mjs preview --host <host> --core-cli <loaded-core-cli> --inline ...`, then its `apply` command only after approval. It derives verified inventory and doctor state directly; do not hand-build preflight JSON. Keep its receipt path and approval digest in agent state only.
