...
exports.DECISION_ASSESSMENT_CONTRACT = void 0;
exports.canonicalTerms = canonicalTerms;
exports.deriveSearchTerms = deriveSearchTerms;
exports.decisionAssessmentContract = decisionAssessmentContract;
exports.prepareDecisionCheck = prepareDecisionCheck;
exports.prepareDecisionCompare = prepareDecisionCompare;
...
exports.DECISION_ASSESSMENT_CONTRACT = 'context-decision-assessment/v1';
...
function selectDecisionCandidates(rows, options) {
    ...
    const ranked = rows.map((row, index) => {
        ...
        if (exact) {
            ...
            else if (mode === 'legacy' && (0, common_1.scopesOverlap)(row.scope, scope)) {
                score += 20;
...
function readDecisionBodies(root, area, selected, mandatory, proposal) {
...
function prepareDecisionEvidence(root, options, mode) {
    if (mode === 'scoped')
        (0, common_1.check)(typeof options.scope === 'string' && typeof options.decisionKey === 'string', 'usage_invalid', 'Compare requires both scope and decisionKey.');
    ...
    const exact = options.scope !== undefined, scope = exact ? (0, common_1.canonicalScope)(options.scope) : null, key = exact ? (0, common_1.canonicalKey)(options.decisionKey) : null, area = decisionArea(root), allRows = (0, documents_1.parseAreaIndex)(area.text).current;
    const rows = mode === 'scoped' ? allRows.filter(r => (0, common_1.scopesOverlap)(r.scope, scope)) : allRows;
    ...
    return { comparison, currentLinks, known, exact,
        deterministic: { exact_slot: extract(current.filter(x => exact && x.scope === scope && x.decision_key === key)), scope_overlap: extract(current.filter(x => exact && x.decision_key === key && x.scope !== scope && (0, common_1.scopesOverlap)(x.scope, scope))) },
...
function decisionAssessmentContract(withRevisit = true) {
...
function comparisonTransport(comparison, known) {
    return { schema: 'context-decision-comparison-delta/v1', proposal: comparison.proposal, current: comparison.current.map((record) => {
...
function prepareDecisionCheck(root, options) {
    ...
    const result = { schema: 'context-decision-check/v1', coverage: exact ? 'exact_slot' : 'discovery_only', comparison_input: comparison, input_digest: (0, common_1.canonicalDigest)(comparison), deterministic, assessment_contract: assessment, retrieval, warning: comparisonWarning, physical_write: false };
    ...
    if (options.knownCurrent !== undefined) {
        ...
        result.transport_digest = (0, common_1.canonicalDigest)(delta);
        ...
        (0, common_1.check)(Buffer.byteLength((0, common_1.canonicalJson)(result)) <= 32768, 'comparison_too_large', 'Decision check delta exceeds its output byte limit.', {}, common_1.EXIT.conflict);
    }
    return result;
}
function prepareDecisionCompare(root, options) {
    ...
    const result = {
        ...
        assessment_contract_ref: exports.DECISION_ASSESSMENT_CONTRACT,
        ...
        hydrated_input_digest: (0, common_1.canonicalDigest)(comparison), transport_digest: (0, common_1.canonicalDigest)(transport),
        warnings, physical_write: false,
    };
    (0, common_1.check)(Buffer.byteLength((0, common_1.canonicalJson)(result)) <= 32768, 'comparison_too_large', 'Decision compare exceeds its output byte limit.', {}, common_1.EXIT.conflict);
    return result;
}
