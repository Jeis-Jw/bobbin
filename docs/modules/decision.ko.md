...
Bobbin 2.3.0에 포함된 내부 모듈이며 별도 플러그인이 아니다. 설치·설정은 [시작 안내](../../README.ko.md)와 단일 `$bobbin:init` 진입점을 사용한다. SNAP·OBS·ARCHIVE는 기본 제공하며 나머지 owner는 프로젝트에서 선택한다.
...
기본 경로는 메타데이터 `search` → 범위 지정 `compare` → 승인된 `record`다. [Node API](../node-api.ko.md#범위를-지정한-결정-비교)를 따른다. 기존 `check`는 호환 지원한다.
