...
- 모든 semantic owner는 같은 Bobbin 패키지의 core를 사용한다. 별도 plugin dependency를 설치하지 않으며 공개 버전은 `2.3.0`, protocol은 기존 `context-common/v2`다. 역사적 artifact schema와 scope는 리브랜딩하지 않는다.
