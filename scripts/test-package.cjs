...
(async () => {
    ...
    const compared = wrapped('decision/scripts/decision_cli.mjs', ['compare', '--statement', 'Use local files.', '--scope', 'package', '--decision-key', 'storage']);
    assert.equal(compared.result.schema, 'context-decision-compare/v1');
    assert.deepEqual(compared.result.comparison.current, checked.result.comparison_input.current);
    assert.equal(wrapped('decision/scripts/decision_cli.mjs', ['schema']).result.comparison_contract.schema, compared.result.assessment_contract_ref);
    assert.equal(wrapped('context/scripts/context_cli.mjs', ['doctor']).result.record_count, 1);
    const guidance = run(process.execPath, ['--test', path.join(root, 'tests/node/guidance.test.cjs'), path.join(root, 'tests/node/decision-navigation.test.cjs'), path.join(root, 'tests/node/decision-reuse.test.cjs')], { env: { ...process.env, BOBBIN_TEST_PACKAGE_ROOT: packageRoot } });
    fs.writeFileSync(path.join(consumer, 'consumer.ts'), "import {createBobbin,createCandidate,ReadResult,DecisionCheckOptions,DecisionCompareOptions} from '@bobbin/context'; const b=createBobbin({vault:'/an/existing/directory'}); const c=createCandidate({kind:'snapshot',title:'Handoff',summary:'Resume',ownerInputs:{current_context:'Continue',open_items:['Check'],next_steps:['Run']}}); const read:Promise<ReadResult>=b.read('ctx_...'); const check:DecisionCheckOptions={statement:'Review storage.',knownCurrent:[{id:'ctx_00000000000040008000000000000001',sha256:'sha256:'+'0'.repeat(64)}]}; void b.checkDecision(check); const compare:DecisionCompareOptions={...check,scope:'package',decisionKey:'storage'}; void b.compareDecision(compare); void c; void read;\n");
