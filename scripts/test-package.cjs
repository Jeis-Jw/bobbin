...
(async () => {
    ...
    fs.writeFileSync(path.join(consumer, 'consumer.ts'), "import {createBobbin,createCandidate,ReadResult,DecisionCheckOptions} from '@bobbin/context'; const b=createBobbin({vault:'/an/existing/directory'}); const c=createCandidate({kind:'snapshot',title:'Handoff',summary:'Resume',ownerInputs:{current_context:'Continue',open_items:['Check'],next_steps:['Run']}}); const read:Promise<ReadResult>=b.read('ctx_...'); const check:DecisionCheckOptions={statement:'Review storage.',knownCurrent:[{id:'ctx_00000000000040008000000000000001',sha256:'sha256:'+'0'.repeat(64)}]}; void b.checkDecision(check); void c; void read;\n");
